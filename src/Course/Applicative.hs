...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering f xs = foldRight (\x acc ->
                             lift2 (\y -> if y then (x :.) else id)
                             (f x) acc
                             ) (pure Nil) xs
